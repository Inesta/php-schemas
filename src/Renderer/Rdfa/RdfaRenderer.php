...
/**
 * Renders schemas as HTML with RDFa markup.
 *
...
final class RdfaRenderer implements RendererInterface
{
    public function render(SchemaTypeInterface $schema): string
    {
        ...
        $html = "<div vocab=\"{$schema->getContext()}/\" typeof=\"{$type}\">\n";
    ...
    private function renderProperty(string $property, mixed $value): string
    {
        if ($value instanceof SchemaTypeInterface) {
            return "  <div property=\"{$property}\">\n"
                   . $this->indentHtml($value->toRdfa(), 2) . "\n"
                   . "  </div>\n";
        }
        ...
        return "  <span property=\"{$property}\">{$escapedValue}</span>\n";
    }
