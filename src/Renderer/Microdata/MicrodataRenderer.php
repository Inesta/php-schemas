...
/**
 * Enhanced Microdata renderer with configuration options.
 *
 * Renders schemas as HTML with Microdata markup, supporting various
...
final class MicrodataRenderer implements RendererInterface
{
    ...
    public function render(SchemaTypeInterface $schema): string
    {
        ...
        $html = "<{$element} itemscope itemtype=\"{$schema->getContext()}/{$type}\">{$newline}";
    ...
    private function renderProperty(string $property, mixed $value, int $indentLevel = 0): string
    {
        ...
        if ($value instanceof SchemaTypeInterface) {
            return "{$indent}<div itemprop=\"{$property}\">{$newline}"
                   . $this->indentHtml($value->toMicrodata(), $indentLevel + 1) . $newline
                   . "{$indent}</div>{$newline}";
        }
        ...
        if ($this->includeMetaElements && $this->isMetaProperty($property)) {
            return "{$indent}<meta itemprop=\"{$property}\" content=\"{$escapedValue}\">{$newline}";
        }

        return "{$indent}<{$element} itemprop=\"{$property}\">{$escapedValue}</{$element}>{$newline}";
    }
